...
from transformers.models.mpt.modeling_mpt import MptConfig, MptAttention
from kernel.palu_attention import LlamaPaluAttention, LlamaPaluAttention_noRoPE
from mpt_temp import MptPaluAttention
...
def build_attention(args):
    ...
    logging.info(f"Creating MPT Attention, dtype: {dtype}, device: {device}")
    config = AutoConfig.from_pretrained("mosaicml/mpt-7b")
    config.max_seq_len = 300000
    attention = MptAttention(config).to(device, dtype)
...
def build_attention_palu(args):
    ...
    logging.info(f"Creating Palu MPT Attention, dtype: {dtype}, device: {device}")
    config = AutoConfig.from_pretrained("mosaicml/mpt-7b")
    config.max_seq_len = 300000
    ...
    attention_palu = MptPaluAttention(config).to(device, dtype)
    
    return attention_palu, config

def profile_tpot(model, cache_size_k, cache_size_v, cache_type=torch.float16, bsz=1, prompt_len=1024, repeats=100,
                 cache_graph=False, torch_profile=False, outfile="", args=None):
    ...
    input_token = torch.randn((bsz, 1, hidden_dim), dtype=torch.float16, device=device) # only input 1 token at a time
    # TODO: Add position_bias
    position_bias = None
    ...
    with torch.no_grad():
        with torch.cuda.stream(s):
            for _ in range(25):
                _ = model(input_token, position_bias=None, past_key_value=past_key_value)
    ...
    if cache_graph:
        with torch.no_grad():
            ...
            with torch.cuda.graph(graph):
                out = model(input_token, position_bias=None, past_key_value=past_key_value)
            
        def generate(new_input_token, position_bias, past_key_value):
            input_token.copy_(new_input_token)
            graph.replay()
            return out
    else:
        def generate(new_input_token, position_bias, past_key_value):
            out = model(new_input_token, position_bias=None, past_key_value=past_key_value)
            return out

    new_input_token = torch.randn((bsz, 1, hidden_dim), dtype=torch.float16, device=device) # only input 1 token at a time
    with torch.no_grad():
        from torch.cuda import nvtx
        a = nvtx.range_start("Palu")
        generate(new_input_token, position_bias=None, past_key_value=past_key_value)
        nvtx.range_end(a)
...
def main(args):
    ...
    if args.palu:
        ...
        profile_tpot(attention, cache_size_k, cache_size_v, torch.float16, bs, args.prompt_len, args.repeats, args.cache_graph, args.torch_profile, "tpot_palu_fp16", args)
    else:
        ...
        profile_tpot(attention, cache_size, cache_size, torch.float16, bs, args.prompt_len, args.repeats, args.cache_graph, args.torch_profile, "tpot_fp16", args)
