...
def test_palu_attention_inherit_no_fusion(config):
    ...
    attention = MptAttention(config)
    palu_attention = MptPaluAttention.from_attention(attention, config, no_fusion=True)

    # q, k, v, o proj
    hidden_states = torch.randn(bsz, seq_len, config.hidden_size)
    mixed_qkv = attention.Wqkv(hidden_states)
    query_states, key_states, value_states = mixed_qkv.chunk(3, dim=2)
    out_states = attention.out_proj(hidden_states)
    torch.testing.assert_close(query_states, palu_attention.q_proj(hidden_states))
    torch.testing.assert_close(key_states, palu_attention.k_proj(hidden_states))
    torch.testing.assert_close(value_states, palu_attention.v_proj(hidden_states))
    torch.testing.assert_close(out_states, palu_attention.out_proj(hidden_states))
...
def test_palu_attention_inherit_fusion(config):
    ...
    attention = MptAttention(config)
    palu_attention = MptPaluAttention.from_attention(attention, config)
    ...
    print(attention.Wqkv)
    print(palu_attention.q_proj)
    print(palu_attention.k_proj)
    print(palu_attention.v_proj)
    print(attention.out_proj)
    print(palu_attention.out_proj)
    print(palu_attention)
    
    # attn_output
    # original
    inputs = torch.randn(1, q_len, hidden_dim)
    value_states = value_states.view(1, q_len, num_heads, head_dim).transpose(1, 2)
    ...
    torch.testing.assert_close(ori_output, fused_o_output, rtol=1e-3, atol=1e-3)
...
if __name__ == "__main__":
    _set_random_seed()
    _config = AutoConfig.from_pretrained("mosaicml/mpt-7b")
