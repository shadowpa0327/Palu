...
def run_benchmark(args):
    ...
    configs.append(
        triton.testing.Benchmark(
            ...
            x_vals=args.n,
            line_arg="provider",
            line_vals=["fp16", "mix-pre-split-K"],
            line_names=["fp16", "mix-pre-split-K"],
            styles=[("green", "--"), ("blue", "-")],
            ylabel="us",
            plot_name=f"mix_pre_bench-k{args.k}",
            args={
                "m": args.m,
                "k": args.k,
                #"n": args.n,
    ...
    def bench_gemm(m, k, n, group_size, provider):
        warmup = 25
        rep = 100
        ...
        if provider == 'fp16':
            ...
            b = make_tensor(k, n, dtype=torch.float16)
            def fn(): return torch.matmul(a, b)
            ms, min_ms, max_ms = triton.testing.do_bench(
                fn, quantiles=quantiles, warmup=warmup, rep=rep)
        
        if provider == 'mix-pre-split-K':
            g = k // group_size
            #bits_per_features = 32 // 8
            a = make_tensor(m, k, dtype=torch.float16)
            b = make_tensor(k//8, n, dtype=torch.int32)
            #c = make_tensor(m, n, dtype=torch.float16)
            zeros = make_tensor(k//128, n//8, torch.int32)
            scales = make_tensor(k//128, n, torch.float16)
            def fn(): return matmul_split_k(a, b, scales, zeros)
            ms, min_ms, max_ms = triton.testing.do_bench(
                fn, quantiles=quantiles, warmup=warmup, rep=rep)
    ...
    bench_gemm.run(print_data=True, show_plots=True, save_path="results/")

if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description="GPT-Q benchmark")
